...
int main()
{
    ...
    for(int i = 0; i < 26; i++)
    {
        if(arr_big[i] != 0)
        {
            arr_small[count2++] = arr_big[i];	/*判断对应字母是否存在*/
            {
...
void huffmanTree(int w[],int huffman[][4],int n)
{
    ...
    for(int i = n; i < 2 * n - 1; i++)
    {
        ...
        huffman[i][3] = huffman[i1][3] + huffman[i2][3];//产生新的中间节点
        huffman[i][1] = i1;//更新中间节点的左子节点
        huffman[i][2] =i2;//更新中间节点的右子节点
        huffman[i1][0] = i ;//更新父节点
        huffman[i2][0] = i;//更新父节点
    }
}
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  /*判断该节点是否存在*/
        {
            break;
        }
        if(huffman[i][3] < min && huffman[i][0] == -1)  /*如果小于上一轮的最小值*/
        {
            min = huffman[i][3];
            ii[0] = i;
        }
    ...
    for (j = 0; j < 2 * n - 1; j++)
    {
        if (huffman[j][3] == -1 && huffman[j][0] == -1) //如果对应结点不存在
        {
            break;
        }
        if (j != ii[0])
        {
            if (huffman[j][3] < min_2 && huffman[j][0] == -1)  // 如果权重小于上一轮最小的权重并且大于实际最小的权重，即找到最新的权重第二最小的结点
            {
                  min_2 = huffman[j][3];
                  ii[1] = j;
            }
...
void HuffmanCode(int i,int huffman[][4],int n)
{
    ...
    while(father != -1)
    {
        if (huffman[father][1] == i)//判断当前结点的父结点左子树是否为当前结点
        {
         ...
         else if (huffman[father][2] == i)
        {
            code[start] = '1';//子结点是父结点的右子树，编码为1
        }
         i = father;//往上朔源，更新当前结点
        father = huffman[i][0];		//同理（当前结点更新后），更新当前结点的父亲结点
        start++;	//更新编码位置
    }
    code[start]='\0';   //编码结束符
...
### 运行截图：
![HUFFMAN2](https://github.com/puppy112358/Geek_Appraisal/assets/115990103/c3443973-e9ab-4411-8db1-7fe8f8b23441)

![HUFFMAN1](https://github.com/puppy112358/Geek_Appraisal/assets/115990103/a04a472a-9017-4a91-a42d-89bbea0132de)
...
1. 当Huffman树的叶结点为n时，整个生成树的结点总数为何是2n-1？因为叶节点为n时，一共进行n-1 次Huffman树的生成，每一次都会新生成一个新的中间节点，共有n-1个中间节点，所以节点总数为2n-1.
3. 该代码可以对大写字母的字符串进行huffman编码，如果要读小写字母字符串进行编码，应该如何修改？如果想要对同时具有大写和小写字母的字符串都进行编码呢（写出思路即可）？1.代码第31行处0x41+i修改为0x61+i;2.第29行代码改成j<26||(j>=32&&j<=57)
4. 使用画图工具画出上述两个Huffman生成树的生成过程
![形成图1](https://github.com/puppy112358/Geek_Appraisal/assets/115990103/4d14c38f-262c-4b7b-9e81-19d7066b818b)
![形成图2](https://github.com/puppy112358/Geek_Appraisal/assets/115990103/a33d71c6-6d46-4946-a0cd-2e3f8e9773ab)
